    ...
    <Dialog open={showChangelogDialog} onOpenChange={onClose}>
      <DialogContent className="w-11/12 max-w-3xl p-0 h-[80vh] overflow-hidden flex flex-col">
        <div className="px-6 pt-6 pb-4 border-b border-border flex items-center justify-between">
          <h1 className="text-xl font-semibold">{t("changelog.title")}</h1>
          <a
          ...
          >
            {t("changelog.viewAll")} <ExternalLink className="w-3 h-3" />
          </a>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {loading ? (
            <div className="text-sm text-muted-foreground font-mono">{t("changelog.loading")}</div>
