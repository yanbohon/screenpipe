...
const TABS: ReadonlyArray<{ value: HistoryTab; labelKey: string }> = [
  { value: "active", labelKey: "chat.history.tabs.active" },
  { value: "archived", labelKey: "chat.history.tabs.archived" },
  { value: "all", labelKey: "chat.history.tabs.all" },
];
...
export function ChatHistoryView({
  ...
  const [bulkPending, setBulkPending] = useState<
    null | "archiving" | "restoring" | "deleting"
  >(null);
  const [rowPendingIds, setRowPendingIds] = useState<Set<string>>(
    () => new Set(),
  );
  ...
  const handleMoveToGroup = useCallback(
    async (id: string, group: string | undefined) => {
  ...
  const Row = ({ conv }: { conv: ConversationMeta }) => {
    ...
    const currentGroup = (currentSidebarGroup ?? conv.pipeContext?.pipeName)
      ?.trim()
      .toLowerCase();
        ...
        <div className="h-5 w-5 flex items-center justify-center relative">
          ...
          >
            <Checkbox
              ...
              aria-label={
                selected
                  ? t("chat.common.deselectChat")
                  : t("chat.common.selectChat")
              }
            />
          </div>
        </div>
            ...
            >
              ...
              >
                ...
                >
                  ...
                  {selected
                    ? t("chat.common.deselect")
                    : t("chat.common.select")}
                </DropdownMenuItem>
                  ...
                  onSelect={(e) => {
                    void (async () => {
                      ...
                      try {
                        ...
                        toast({
                          title: t("chat.history.updateFailedTitle"),
                          description: t(
                ...
                >
                  ...
                  {conv.pinned ? t("chat.common.unpin") : t("chat.common.pin")}
                </DropdownMenuItem>
                <DropdownMenuSub>
                      ...
                      {currentSidebarGroup && (
                        <>
                          ...
                          <DropdownMenuItem
                            ...
                            onSelect={() =>
                              void handleMoveToGroup(conv.id, undefined)
                            }
                          >
        ...
        <div className="sticky top-0 z-10 bg-background">
          <div className="max-w-4xl mx-auto px-6 pt-6">
            ...
            <div
              ...
              role="tablist"
              aria-label={t("chat.history.filterAria")}
            >
              {TABS.map((tabOption) => {
                ...
                >
                    ...
                    {(() => {
                      ...
                      return (
                        <Checkbox
                          ...
                          aria-label={
                            allVisibleSelected
                              ? t("chat.common.clearSelection")
                              : t("chat.common.selectAllVisibleChats")
                          }
                        />
                      );
                    })()}
                    ...
                    {bulkPending && (
                      <span className="inline-flex items-center gap-1 text-xs text-muted-foreground/70">
                        ...
                        {bulkPending === "archiving"
                          ? t("chat.history.bulk.archiving")
                          : bulkPending === "restoring"
                            ? t("chat.history.bulk.restoring")
                            : t("chat.history.bulk.deleting")}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    {(() => {
                      ...
                      const canArchive = ids.some(
                        (id) => !visibleById.get(id)?.hidden,
                      );
                      const canRestore = ids.some(
                        (id) => visibleById.get(id)?.hidden,
                      );
                      const showArchive =
                        tab === "active"
                          ...
                          {showArchive && (
                            <button
                              ...
                              onClick={async () => {
                                ...
                                const result = await bulkSetHidden(
                                  idsToArchive,
                                  true,
                                );
                                setBulkPending(null);
                                if (result.failed.length > 0) {
                                  toast({
                                    title: t("chat.history.archiveFailedTitle"),
                                    description: t("chat.common.failedCount", {
                          ...
                          {showRestore && (
                            <button
                              ...
                              onClick={async () => {
                                ...
                                if (result.failed.length > 0) {
                                  toast({
                                    title: t("chat.history.restoreFailedTitle"),
                                    description: t("chat.common.failedCount", {
                                      count: result.failed.length,
                                    }),
                                  });
                  ...
                  <div className="relative flex-1 min-w-0">
                    ...
                    {query.trim() && (
                      <button
                        ...
                        aria-label={t("chat.common.clearSearch")}
                        title={t("chat.common.clear")}
                        onClick={() => setQuery("")}
                      >
                        <X className="h-3.5 w-3.5" />
                      </button>
            ...
            <div className="divide-y divide-border/30">
              ...
              {loadingMore && (
                <div className="flex items-center justify-center py-4 text-xs text-muted-foreground">
                  <Loader2
                    className="h-3.5 w-3.5 animate-spin mr-2"
                    aria-hidden
                  />
      ...
      >
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>
              {deleteIds.length > 1
                ? t("chat.history.deleteTitleMany")
                : t("chat.history.deleteTitleOne")}
            </DialogTitle>
            <DialogDescription>
              {deleteIds.length > 1
                ? t("chat.history.deleteDescriptionMany", {
                    count: deleteIds.length,
                  })
                : t("chat.history.deleteDescriptionOne")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (failed.length > 0) {
                  toast({
                    title: t("chat.history.deleteFailedTitle"),
                    description: t("chat.common.failedCount", {
                      count: failed.length,
                    }),
                  });
