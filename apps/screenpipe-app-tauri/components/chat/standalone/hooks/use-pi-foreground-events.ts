...
export function usePiForegroundEvents({
  ...
  const { t } = useI18n();
  const piAgentErrorTitle = t("chat.toasts.piAgentError");
  const invalidatedAuthTokenMessage = t("chat.errors.sessionExpired");
  const modelRequiresBusinessMessage = t("chat.errors.modelRequiresBusiness");
  const midResponseMessage = t("chat.errors.midResponse");
  const serverErrorMessage = t("chat.errors.serverError");
  const pipeCompletedNoOutputMessage = t("chat.pipe.completedNoOutput");
  const tRef = useRef(t);
  const piAgentErrorTitleRef = useRef(piAgentErrorTitle);
  const invalidatedAuthTokenMessageRef = useRef(invalidatedAuthTokenMessage);
  const modelRequiresBusinessMessageRef = useRef(modelRequiresBusinessMessage);
  const midResponseMessageRef = useRef(midResponseMessage);
  const serverErrorMessageRef = useRef(serverErrorMessage);
  const pipeCompletedNoOutputMessageRef = useRef(pipeCompletedNoOutputMessage);

  useEffect(() => {
    tRef.current = t;
    piAgentErrorTitleRef.current = piAgentErrorTitle;
    invalidatedAuthTokenMessageRef.current = invalidatedAuthTokenMessage;
    modelRequiresBusinessMessageRef.current = modelRequiresBusinessMessage;
    midResponseMessageRef.current = midResponseMessage;
    serverErrorMessageRef.current = serverErrorMessage;
    pipeCompletedNoOutputMessageRef.current = pipeCompletedNoOutputMessage;
  }, [
    invalidatedAuthTokenMessage,
    midResponseMessage,
    modelRequiresBusinessMessage,
    piAgentErrorTitle,
    pipeCompletedNoOutputMessage,
    serverErrorMessage,
    t,
  ]);
  ...
  useEffect(() => {
    ...
    const handlePiEventData = (payload: unknown) => {
      ...
      if (
        ...
        const errorStr = stringValue(
          data.finalError,
          tRef.current("chat.errors.requestFailedAfterRetries"),
        );
        ...
        if (quotaErrorType === "daily" || quotaErrorType === "rate") {
          ...
          if (piMessageIdRef.current) {
            ...
            const content =
              quotaErrorType === "daily"
                ? buildDailyLimitMessage(errorStr, tRef.current)
                : buildRateLimitMessage(errorStr, tRef.current);
            setMessages((prev) =>
        ...
        if (piMessageIdRef.current) {
          ...
          if (quotaErrorType === "daily" || quotaErrorType === "rate") {
            if (quotaErrorType === "daily") {
              ...
              const content = buildRateLimitMessage(fullError, tRef.current);
              setMessages((prev) =>
                prev.map((m) => (m.id === msgId ? { ...m, content } : m)),
              );
            }
              ...
              setMessages((prev) =>
                prev.map((m) =>
                  m.id === msgId
                    ? {
                        ...m,
                        content: tRef.current("chat.message.errorWithMessage", {
                          message:
                            fullError ||
                            tRef.current("chat.errors.somethingWentWrong"),
                        }),
                      }
                    : m,
                ),
        ...
        const errMsg = stringValue(
          ...
          stringValue(data.message.error, tRef.current("chat.toasts.unknownError")),
        );
        ...
        if (piMessageIdRef.current) {
          ...
          if (authTokenInvalidated) {
            setMessages((prev) =>
              prev.map((m) =>
                m.id === msgId
                  ? { ...m, content: invalidatedAuthTokenMessageRef.current }
                  : m,
              ),
            );
            ...
            setMessages((prev) =>
              prev.map((m) =>
                m.id === msgId
                  ? {
                      ...m,
                      content: buildDailyLimitMessage(errMsg, tRef.current),
                    }
                  : m,
              ),
            );
            ...
            setMessages((prev) =>
              prev.map((m) =>
                m.id === msgId
                  ? {
                      ...m,
                      content: buildRateLimitMessage(errMsg, tRef.current),
                    }
                  : m,
              ),
            );
        ...
        if (piMessageIdRef.current && !isPipeWatch) {
          ...
          if (agentEndError && !content) {
            ...
            if (isInvalidatedAuthTokenError(errStr)) {
              void handleInvalidatedAuthToken();
              content = invalidatedAuthTokenMessageRef.current;
            } else if (quotaErrorType === "daily") {
              content = buildDailyLimitMessage(errStr, tRef.current);
            } else if (quotaErrorType === "rate") {
              content = buildRateLimitMessage(errStr, tRef.current);
          ...
          setMessages((prev) => {
            ...
            if (!content && hasNonTextBlocks) {
              ...
              if (lastErr && lastErrKind === "daily") {
                ...
                content = buildDailyLimitMessage(lastErr, tRef.current);
              } else if (lastErr && lastErrKind === "rate") {
                content = buildRateLimitMessage(lastErr, tRef.current);
              } else if (lastErr) {
                content =
                  buildProviderErrorMessage(
                    lastErr,
        ...
        const errorStr = stringValue(
          data.error,
          tRef.current("chat.toasts.unknownError"),
        );
        emitSessionActivity({ status: "error", lastError: errorStr });
        ...
        if (piMessageIdRef.current) {
          ...
          if (quotaErrorType === "daily" || quotaErrorType === "rate") {
            if (quotaErrorType === "daily") {
              ...
              const content = buildRateLimitMessage(errorStr, tRef.current);
              setMessages((prev) =>
                prev.map((m) => (m.id === msgId ? { ...m, content } : m)),
              );
            }
              ...
              setMessages((prev) =>
                prev.map((m) =>
                  m.id === msgId
                    ? {
                        ...m,
                        content: midResponseMessageRef.current,
                        retryPrompt: lastUserMessageRef.current || undefined,
                      }
                    : m,
                ),
              ...
              setMessages((prev) =>
                prev.map((m) =>
                  m.id === msgId
                    ? {
                        ...m,
                        content: serverErrorMessageRef.current,
                        retryPrompt: lastUserMessageRef.current || undefined,
                      }
                    : m,
                ),
        ...
        if (piMessageIdRef.current?.startsWith("pipe-")) {
          const msgId = piMessageIdRef.current;
          const content =
            piStreamingTextRef.current || pipeCompletedNoOutputMessageRef.current;
          const blocksSnapshot = [...piContentBlocksRef.current];
          setMessages((prev) =>
            prev.map((m) =>
              m.id === msgId
    ...
    const setup = async () => {
      ...
      unlistenLog = await registerPiLogListener({
        getAgentErrorTitle: () => piAgentErrorTitleRef.current,
        getImagesUnsupportedMessage: () =>
          tRef.current("chat.errors.imagesUnsupported"),
        getModelNotFoundMessage: (line) =>
          tRef.current("chat.errors.modelNotFound", { error: line }),
        getModelRequiresBusinessMessage: () =>
          modelRequiresBusinessMessageRef.current,
        getOllamaConnectionMessage: () =>
          tRef.current("chat.errors.ollamaConnection"),
        getRateLimitedMessage: () =>
          tRef.current("chat.errors.rateLimitedSwitchModel"),
        isMounted: () => mounted,
        cancelStreamingMessageRender,
        piMessageIdRef,
        setMessages,
