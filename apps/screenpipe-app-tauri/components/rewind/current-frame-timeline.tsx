	...
	useEffect(() => {
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			toast({
				...
				description: t("rewind.frame.toast.allFrameTextCopied"),
			});
		};

		el.addEventListener("keydown", handleKeyDown);
		return () => el.removeEventListener("keydown", handleKeyDown);
	}, [frameContext?.text, textPositions, t]);
						...
						<div className="bg-card border border-border p-8">
							...
							<div className="text-center space-y-3">
								<h3 className="text-xl font-mono font-semibold text-foreground uppercase tracking-wide">
									{t("rewind.frame.empty.title")}
								</h3>
								<p className="text-sm font-mono text-muted-foreground leading-relaxed">
									{t("rewind.frame.empty.description")}
								</p>
							</div>
							{onNavigate && (
								<div className="mt-8 flex gap-2">
									<button onClick={() => onNavigate("prev")} disabled={!canNavigatePrev}
										...
										<ChevronLeft className="w-4 h-4" /> {t("rewind.frame.nav.previous")}
									</button>
									<button onClick={() => onNavigate("next")} disabled={!canNavigateNext}
										className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-background hover:bg-accent disabled:opacity-30 disabled:cursor-not-allowed border border-border text-foreground text-sm font-mono uppercase transition-colors">
										{t("rewind.frame.nav.next")} <ChevronRight className="w-4 h-4" />
									</button>
								</div>
							)}
						</div>
						...
						<div className="flex items-start gap-4">
							...
							<div className="min-w-0 space-y-2">
								...
								<h3 className="text-lg font-mono font-semibold uppercase tracking-wide">
									{t("rewind.frame.paused.title")}
								</h3>
								<p className="text-sm font-mono leading-relaxed text-white/65">
									{t("rewind.frame.paused.description")}
								</p>
							</div>
						</div>
							...
							{frameText && (
								<button
									...
									onClick={() => {
										...
										toast({
											title: t("rewind.frame.toast.copiedText"),
											description: t("rewind.frame.toast.frameTextCopied"),
										});
									}}
								...
								>
									...
									{t("rewind.frame.actions.copyText")}
								</button>
							)}
							{onNavigate && (
								<>
			...
			{contextMenuOpen && contextMenuPositionRef.current && debouncedFrame?.frameId && (
				...
				>
					...
					{(() => {
						...
						return (
							<button
								...
								onClick={() => {
									...
									if (text) {
										commands.copyTextToClipboard(text).catch(() => {});
										toast({
											title: t("rewind.frame.toast.copiedSelection"),
											description: t("rewind.frame.toast.selectedTextCopied"),
										});
									}
							...
							>
								...
								<span className="flex-1 min-w-0 truncate">{t("rewind.frame.actions.copySelectedText")}</span>
							</button>
						);
					})()}
					<button
						...
						title={t("rewind.frame.actions.copyImageTitle")}
					>
						<ImageIcon className="w-4 h-4 shrink-0" />
						<span className="flex-1 min-w-0 truncate">{t("rewind.frame.actions.copyImage")}</span>
					...
					<button
						...
						title={t("rewind.frame.actions.copyDeeplinkTitle")}
					>
						<Link2 className="w-4 h-4 shrink-0" />
						{t("rewind.frame.actions.copyDeeplink")}
					</button>
					<button
						type="button"
						className="block w-full text-left px-3 py-2 -my-px first:mt-0 last:mb-0 border-0 outline-none ring-0 bg-transparent hover:bg-foreground hover:text-background text-foreground flex items-center gap-2 cursor-pointer transition-colors duration-150"
