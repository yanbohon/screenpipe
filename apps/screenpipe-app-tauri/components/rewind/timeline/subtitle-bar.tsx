		...
		return (
			<div className="flex justify-center">
				<button
					...
					title={t("rewind.subtitle.showCaptions")}
				>
					<MessageSquareText className="w-3.5 h-3.5" />
					<span className="text-[10px] font-medium">{t("rewind.subtitle.captions")}</span>
				</button>
			</div>
		);
	}
		...
		>
			...
			<div className="flex justify-end mb-1">
				<button
					...
					title={t("rewind.subtitle.hideCaptions")}
				>
					<X className="w-3 h-3" />
					<span>{t("rewind.subtitle.hide")}</span>
				</button>
			</div>
			<div
				ref={scrollRef}
			...
			>
				...
				{transcriptionPaused && (
					<div className="flex items-center gap-1.5 text-[10px] text-amber-500/80">
						...
						<span>{t("rewind.subtitle.transcriptionPaused")}{meetingApp ? ` (${meetingApp})` : ""}</span>
					</div>
				)}
				...
				<div className={`flex items-center justify-center gap-1 transition-all duration-200 overflow-hidden ${isHovered ? "max-h-6 opacity-100 pt-0.5" : "max-h-0 opacity-0"}`}>
					...
					<span className="text-[10px] text-muted-foreground/60">{t("rewind.subtitle.fullTranscriptHint")}</span>
				</div>
			</div>
		</div>
	);
...
function SubtitleLine({
	...
	const speakerLabel = entry.is_input
		? t("rewind.subtitle.you")
		: entry.speaker_name || entry.device_name || t("rewind.subtitle.speaker");
		...
		>
			...
			{isPending ? (
				<span className="text-sm text-muted-foreground/50 flex-1 flex items-center gap-1.5 italic">
					...
					{t("rewind.subtitle.transcribing")}
				</span>
			) : (
