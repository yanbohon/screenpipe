...
export type Settings = SettingsStore & {
  ...
  fontSize?: FontSize;
  /** Interface language preference. Separate from transcription `languages`. */
  uiLanguage?: Locale;
  ...
  /** Hide model thinking blocks in chat history. Default true so raw reasoning stays collapsed away. */
...
const applyProCloudAudioDefaults = (settings: Settings): Settings => {
  ...
  const oldCloudEngineMigrationAlreadyRan =
    ...
    DEFAULT_SETTINGS.ocrEngine =
      p === "macos"
        ...
        : p === "windows"
          ? "windows-native"
          : "tesseract";
    DEFAULT_SETTINGS.showScreenpipeShortcut =
    ...
    _store = (async () => {
      ...
      try {
        ...
        if (res.status === "ok") {
          ...
          console.warn(
            "get_screenpipe_base_dir failed, using ~/.screenpipe:",
            res.error,
          );
        }
      } catch (e) {
        console.warn(
          "get_screenpipe_base_dir unavailable, using ~/.screenpipe:",
          e,
        );
      }
      if (!baseDir) {
...
function createSettingsStore() {
  const get = async (): Promise<Settings> => {
    ...
    const hasCloudPreset = settings.aiPresets?.some(
      (p: any) =>
        p.id === "screenpipe-cloud" || p.provider === "screenpipe-cloud",
    );
    if (
      settings.aiPresets &&
    ...
    if (
      isLoggedInProUser(settings.user) &&
      !(settings as any)._proCloudAudioDefaultsAppliedV2
    ) {
  ...
  const listen = (callback: (settings: Settings) => void) => {
    return getStore().then((store) => {
      ...
      return store.onKeyChange(
        "settings",
        async (newValue: Settings | null | undefined) => {
          const mySeq = ++seq;
          const next = await hydrateCloudToken(
            newValue || createDefaultSettingsObject(),
          );
...
export const SettingsProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    settings.user?.id,
    settings.user?.clerk_id,
    settings.user?.cloud_subscribed,
    (settings.user as any)?.app_entitled,
    (settings.user as any)?.subscription_plan,
  ]);
